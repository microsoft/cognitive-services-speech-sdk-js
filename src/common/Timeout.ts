...
export class Timeout {
    ...
    public static load(): any {
        ...
        let workerUrl: string | TrustedScriptURL;

        // eslint-disable-next-line arrow-body-style
        const hashCode = (str: string): number => {
            return str.split("").reduce((prevHash: number, currVal: string): number =>
        ...
        const srcSw = (str: string): string => {
            if (hashCode(str) === 2045125951) {
                return str;
            }
            throw new TypeError("invalid setTimeout script url - Timeout.ts");
        };
        ...
        if (!!window.trustedTypes && !!window.trustedTypes.createPolicy) {
            // define the policy
            // eslint-disable-next-line
            const swPolicy: TrustedTypePolicy = window.trustedTypes.createPolicy('swPolicy', {
                createHTML: undefined,
                createScript: undefined,
                createScriptURL: (src: string): string => srcSw(src),
            });
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
            workerUrl = swPolicy.createScriptURL("data:text/javascript;base64," + btoa(workerScript));

        // create our trusted type
        } else {
            // we don't have trusted types or the polyfill didn't load
            workerUrl = srcSw("data:text/javascript;base64," + btoa(workerScript));
        }
